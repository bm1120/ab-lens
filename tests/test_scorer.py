...
def _quality_score(clarity=80, mechanism=80, measurability=80, bias_risk=80) -> QualityScore:
    ...
    weak_axes = [
        ...
        [("clarity", clarity), ("mechanism", mechanism), ("measurability", measurability), ("bias_risk", bias_risk)]
        if val < 60
    ]
    return QualityScore(
        clarity=clarity,
...
def test_scorer_mechanism_empty_returns_zero():
    ...
    fake_score = _quality_score(mechanism=85)  # LLM 은 85 를 줄 것이지만 덮어써야 함

    with patch("src.hypothesis.scorer.call_structured", return_value=fake_score):
        result = score_hypothesis(hyp, api_key="k", provider=LLMProvider.ANTHROPIC)
...
def test_scorer_failed_when_total_lt_70():
    """총점 70 미만이면 passed=False (결정론적 필터로 mechanism=0 강제 시)."""
    # mechanism_path 가 비어있으면 mechanism=0 → total 급감
...
def test_get_weak_axes_returns_below_threshold():
    """get_weak_axes: 임계값(기본 60) 미만 축만 반환."""
    score = QualityScore(
        clarity=75,
        mechanism=40,       # 60 미만 → weak
        measurability=55,   # 60 미만 → weak
        bias_risk=80,
        total=60,
        weak_axes=[],
        passed=False,
...
def test_get_weak_axes_empty_when_all_pass():
    """모든 축이 60 이상이면 빈 리스트 반환."""
    score = QualityScore(
        clarity=70, mechanism=70, measurability=70, bias_risk=70,
        total=70, weak_axes=[], passed=True, rationale="통과",
    )
    assert get_weak_axes(score) == []


def test_get_weak_axes_custom_threshold():
    """사용자 정의 임계값 적용 검증."""
    score = QualityScore(
        clarity=65, mechanism=75, measurability=80, bias_risk=90,
        total=75, weak_axes=[], passed=True, rationale="통과",
    )
    # threshold=70 이면 clarity(65) 가 weak
    weak = get_weak_axes(score, threshold=70)
...
def test_scorer_no_service_context():
    ...
    with patch("src.hypothesis.scorer.call_structured", return_value=fake_score) as mock_cs:
        result = score_hypothesis(
            hyp, api_key="k", provider=LLMProvider.ANTHROPIC, service_context=None
        )

    assert isinstance(result, QualityScore)
    # 컨텍스트 없이도 call_structured 가 호출됨
    assert mock_cs.call_count == 1


def test_scorer_with_service_context_includes_context_in_prompt():
    """service_context 가 있으면 프롬프트에 컨텍스트가 포함된다."""
    hyp = _hypothesis()
    fake_score = _quality_score()
    ctx = ServiceContext(
        service_name="MyShop",
    ...
    with patch("src.hypothesis.scorer.call_structured", return_value=fake_score) as mock_cs:
        score_hypothesis(hyp, api_key="k", provider=LLMProvider.ANTHROPIC, service_context=ctx)

    prompt_arg = mock_cs.call_args.kwargs.get("prompt") or mock_cs.call_args.args[0]
    assert "MyShop" in prompt_arg
