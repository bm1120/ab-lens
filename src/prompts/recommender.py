...
- **의사결정은 효과크기·신뢰구간·실질적 유의성을 우선한다.** p<0.05(통계적 유의성)는 보조 지표일 뿐, 그 자체로 출시 근거가 아니다.
- 효과크기 95% 신뢰구간이 0을 포함하거나(방향 불확실) 비즈니스적으로 의미 있는 최소 크기에 못 미치면, 유의하더라도 '출시'에 신중하라.
- 표본이 매우 크면 사소한 효과도 p<0.05가 된다(p값 과신 금지). 반대로 비유의여도 신뢰구간이 0 근처로 좁으면 '효과 없음'의 유의미한 증거다.
...
- **Base decisions on effect size, confidence intervals, and practical significance first.** p<0.05 (statistical significance) is a secondary signal, not a launch justification on its own.
- If the effect-size 95% CI includes 0 (uncertain direction) or falls short of a business-meaningful minimum, be cautious about "Launch" even when significant.
- With very large samples, trivial effects still hit p<0.05 (don't over-trust p-values). Conversely, a non-significant result with a tight CI near 0 is meaningful evidence of "no effect".
